    ...
    for j in range(injection.shape[1]):
        ...
        for k in range(1, n):
            for l in range(1,k+1):
                time_decay = (1 - np.exp((time[l-1] - time[l]) / taus[j])) * np.exp((time[l]-time[k]) / taus[j])
...
class CRM():
    def __init__(self, primary: bool=True, tau_selection: str = 'per-pair', constraints: str = 'positive'):
        self.primary = primary
        assert (constraints in ('positive','up-to one','sum-to-one','sum-to-one injector')), "Invalid constraints"
        ...
        if tau_selection == 'per-pair':
            ...
            raise ValueError(f'tau_selection must be one of ("per-pair","per-producer"), not {tau_selection}')
    ...
    def get_initial_guess(self, tau_selection: str = ''):
        ...
        x0 = np.concatenate([gains_guess1, tau_guess1, [gains_producer_guess1, tau_producer_guess1]])
        return x0
    ...
    def get_bounds(self, constraints: str = ''):
        ...
        if self.constraints == 'positive':
            ...
            #raise NotImplementedError('sum-to-one is not implemented')
            bounds = ((0, np.inf), ) * n
            def constrain(x):
                x = x[:n_inj]
                return np.sum(x) - 1
    ...
    def fit(self, production, injection, time, num_cores=1, **kwargs):
        ...
        def fit_well(production):
            ...
            residual = lambda x, production: sum( (production - calculate_qhat(x, production))**2 )
            result = optimize.minimize(residual, x0, bounds=bounds, constraints=constraints, 
                                       args=(production,),
                                      **kwargs) 
            return result
        ...
        if num_cores==1:
            ...
            pool = Pool(num_cores)
            results = pool.map(fit_well, production_perwell)
        
        opts_perwell = [ opts(r['x']) for r in results]
#         gains_perwell = [x[0] for x in opts_perwell]
#         tau_perwell = [x[1] for x in opts_perwell]
#         gains_producer = [x[2] for x in opts_perwell]
#         tau_producer = [x[3] for x in opts_perwell]
    ...
    def predict(self, injection = None, time = None):
        gains, tau, gains_producer, tau_producer = (self.gains, self.tau, self.gains_producer, self.tau_producer)
